...
func newUnitsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "",
		Example:     "  doorloop-pp-cli units list",
		Annotations: map[string]string{"pp:endpoint": "units.list", "pp:method": "GET", "pp:path": "/units", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolveRead(cmd.Context(), c, flags, "units", false, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagFilterText, "filter-text", "", "Filter by unit name")
