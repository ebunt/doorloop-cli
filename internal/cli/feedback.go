...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".doorloop-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOORLOOP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOORLOOP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "doorloop-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.doorloop-pp-cli/feedback.jsonl.
When ` + "`DOORLOOP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOORLOOP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  doorloop-pp-cli feedback list
  doorloop-pp-cli feedback list --limit 5
  doorloop-pp-cli feedback list --json`,
