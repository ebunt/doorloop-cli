...
func newLeaseChargesListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "",
		Example:     "  doorloop-pp-cli lease_charges list",
		Annotations: map[string]string{"pp:endpoint": "lease_charges.list", "pp:method": "GET", "pp:path": "/lease-charges", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolveRead(cmd.Context(), c, flags, "lease_charges", false, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagFilterDateFrom, "filter-date-from", "", "Charge date from (YYYY-MM-DD)")
	cmd.Flags().StringVar(&flagFilterDateTo, "filter-date-to", "", "Charge date to (YYYY-MM-DD)")
