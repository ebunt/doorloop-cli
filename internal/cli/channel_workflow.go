...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  doorloop-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  doorloop-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "communication_logs", "expenses", "files", "lease_charges", "lease_credits", "lease_payments", "lease_returned_payments", "leases", "notes", "owners", "portfolios", "properties", "reports", "tasks", "tenants", "units", "users", "vendor_bills", "vendor_credits", "vendors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  doorloop-pp-cli workflow status

  # Show status as JSON
  doorloop-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("doorloop-pp-cli")
			}
