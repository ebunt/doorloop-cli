...
Resource scoping:
  ...
  doorloop-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  doorloop-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  doorloop-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  doorloop-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  doorloop-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"lease_returned_payments": "/lease-returned-payments",
		"leases":                  "/leases",
		"notes":                   "/notes",
		"owners":                  "/owners",
