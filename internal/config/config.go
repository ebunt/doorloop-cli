...
type Config struct {
	...
	Path          string            `json:"-"`
	DoorloopToken string            `json:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("DOORLOOP_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "doorloop-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("DOORLOOP_TOKEN"); v != "" {
		cfg.DoorloopToken = v
		cfg.AuthSource = "env:DOORLOOP_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.DoorloopToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("DOORLOOP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DoorloopToken != "" {
		c.AuthSource = "env:DOORLOOP_TOKEN"
		return "Bearer " + c.DoorloopToken
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"
