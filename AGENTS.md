# DoorLoop Printed CLI Agent Guide

This directory is a generated `doorloop-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
doorloop-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
doorloop-pp-cli which "<capability>" --json
doorloop-pp-cli <command> --help
```
...
```bash
doorloop-pp-cli <command> --help
doorloop-pp-cli <command> --dry-run --agent
```
